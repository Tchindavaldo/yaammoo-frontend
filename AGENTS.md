...
PC où le projet est cloné/pull, dans n'importe quelle session Codex.
...
> `.Codex/hooks/session-start-read.sh` (déclaré dans `.Codex/settings.json`)
...
> `✅ AGENTS.md lu en entier (N l., 16 règles R1→R16) + architecture/README.md (M l.)`
...
> S'applique à l'identique au **backend** (`BACKEND/AGENTS.md`, R19).
