...
- **Catch-up sur `connect` (seul refresh restant)** : à chaque (re)connexion, le handler émet `join_user` puis appelle `refreshNotifications(true)`, `refreshOrders(true)`, `refreshMerchant(false)`, `refreshDriver(false)`, `refreshBonuses(true)` et `refreshFastFoods()` en mode silencieux pour rattraper les events **fire-and-forget** manqués hors-ligne. Les events **fiabilisés** sont, eux, rejoués par le backend (replay + `__eventId` + ACK) — voir plus bas.

> ⚠️ `refreshFastFoods` couvre le **catalogue public** (home, boutique, checkout).
